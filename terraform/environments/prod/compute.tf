locals {
  ...
  lambda_db_url    = try("postgresql://${local.db_creds.username}:${local.db_creds.password}@${aws_db_proxy.main[0].endpoint}:5432/${aws_db_instance.main.db_name}?sslmode=require", "postgresql://${local.db_creds.username}:${local.db_creds.password}@${aws_db_instance.main.address}:5432/${aws_db_instance.main.db_name}?sslmode=require")
