...
async fn handle_request(
    ...
    match (method, path) {
        ...
        ("GET", "/elections") => {
            match shared::list_elections(&state.db_pool).await {
                Ok(elections) => {
        ...
        ("GET", "/elections/all") => {
            match shared::list_all_elections(&state.db_pool).await {
                Ok(elections) => {
                    let result: Vec<_> = elections.into_iter().map(|(id, name, description, status, visibility, password)| {
                        json!({"id": id, "name": name, "description": description, "status": status, "visibility": visibility, "password": password.unwrap_or_default()})
                    }).collect();
        ...
        ("POST", "/my-elections") => {
            ...
            match shared::list_elections_by_creator(&state.db_pool, &payload.user_email, payload.search.as_deref()).await {
                Ok(elections) => {
                    let result: Vec<_> = elections.into_iter().map(|(id, name, description, status, visibility, password)| {
                        json!({"id": id, "name": name, "description": description, "status": status, "visibility": visibility, "password": password.unwrap_or_default()})
                    }).collect();
        ...
        ("GET", "/candidates") => {
            let election_id = event.payload.get("queryStringParameters")
                .and_then(|q| q.get("election_id").and_then(|v| v.as_str()))
                .unwrap_or("");
            match shared::list_candidates(&state.db_pool, election_id).await {
                Ok(candidates) => {
                    let result: Vec<_> = candidates.into_iter().map(|(id, code, name)| {
...
async fn authenticate(
    ...
    match shared::authenticate_user(pool, &payload.email, payload.password.as_deref()).await {
        ...
        Ok(None) => error_response(401, "Correo o contraseña incorrectos. Por favor, verifica tus datos."),
...
async fn register(
    ...
    match shared::create_user(pool, &payload.email, Some(&keys.0), Some(password), "user").await {
        Ok(_) => success_response(json!({"role": "user", "public_key": keys.0, "has_password": true, "has_voted_election": serde_json::Value::Null})),
...
async fn update_role(
    ...
    match shared::update_user_role(pool, &payload.target_email, &payload.new_role).await {
        Ok(_) => {
            let _ = shared::log_audit(pool, "role_updated", &format!("user: {} new_role: {}", payload.target_email, payload.new_role)).await;
...
async fn list_users_handler(
    ...
    match shared::list_users(pool).await {
        Ok(users) => success_response(json!(users.into_iter().map(|(email, role)| json!({"email": email, "role": role})).collect::<Vec<_>>())),
        Err(e) => error_response(500, &format!("Error: {}", e)),
    }
}
...
async fn create_election_handler(
    ...
    match shared::create_election(
        ...
        Ok(_) => {
            let _ = shared::log_audit(pool, "election_created", &format!("election: {}", election_id)).await;
...
async fn get_election_handler(
    pool: &sqlx::PgPool,
    election_id: &str,
) -> ApiGatewayResponse {
    match shared::get_election(pool, election_id).await {
        Ok(Some((id, name, description, is_active))) => {
            success_response(json!({"id": id, "name": name, "description": description, "is_active": is_active}))
        }
...
async fn update_election_handler(
    ...
    match shared::update_election(
        ...
        Ok(_) => {
            let _ = shared::log_audit(pool, "election_updated", &format!("election: {}", payload.election_id)).await;
...
async fn add_candidate_handler(
    ...
    let code = format!("C{}A{}", &payload.election_id[..std::cmp::min(3, payload.election_id.len())].to_uppercase(), uuid_part);

    if let Ok(true) = shared::candidate_exists(pool, &payload.election_id, &payload.name).await {
        return error_response(409, "A candidate with this name already exists in this election");
    }

    match shared::add_candidate(pool, &payload.election_id, &code, &payload.name).await {
        Ok(id) => {
            let _ = shared::log_audit(pool, "candidate_added", &format!("election: {}", payload.election_id)).await;
            success_response(id)
        }
        Err(e) => error_response(500, &format!("Database error: {}", e)),
    }
...
async fn update_candidate_handler(
    ...
    match shared::update_candidate(pool, candidate_id, name).await {
        Ok(_) => {
            let _ = shared::log_audit(pool, "candidate_updated", &format!("candidate: {}", candidate_id)).await;
...
async fn delete_candidate_handler(
    pool: &sqlx::PgPool,
    candidate_id: i64,
) -> ApiGatewayResponse {
    if candidate_id == 0 {
    ...
    match shared::delete_candidate(pool, candidate_id).await {
        Ok(_) => {
            let _ = shared::log_audit(pool, "candidate_deleted", &format!("candidate: {}", candidate_id)).await;
...
async fn register_voter_handler(
    ...
    match shared::register_voter(pool, &payload.election_id, &payload.email, &payload.public_key).await {
        Ok(id) => {
